...
def fasterFrequentWords(text,k): #Bestimmung der Funktion fasterFrequentWords 
  frequentPatterns = [] #Definition frequentPattern
  frequencyArray = computeFrequencies(text,k)   
  maxCount = max(frequencyArray) #Suche nach max Wert im frequencyArray 
  for i in range(4**k): #for-Schleife von 0 bis 4^k
    if frequencyArray[i] == maxCount: #wenn Stelle i im array gleich dem maxcount ist...
      pattern = numberToPattern(i,k) #...dann Berechnung des patterns zu i...
      frequentPatterns.append(pattern) #...und Anhängen zum frequentPattern array
  return frequentPatterns #Wiedergabe frequentPatterns

result=(fasterFrequentWords("ACGCGGCTCTGAAA",2)) #Definieren des Ergebnisses des Algorithmus
print(result)
